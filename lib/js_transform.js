    ...
    return function(req,res,next){
        ...
        if (b_opts.transform) {
          if (options.transform) {
            options.transform = options.transform.concat(b_opts.transform);
          } else {
            options.transform = b_opts.transform;
          }
        }
        ***/
        ...
        //push pkg.transform to browserfiy mdeps  globalTransforms
        if (b_opts.transform) {
          var gl_trs = b_opts.transform.filter(Boolean).filter(bundle._filterTransform);
          if (gl_trs.length) {
            var mdeps_gls = bundle._mdeps.globalTransforms;
            gl_trs.forEach(function(tr){
              if (!~gl_trs.indexOf(tr)) {
                mdeps_gls.push(tr);
              }
            });
          }
