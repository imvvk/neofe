...
function find_file(cwd) {
  ...
  return function(req, res, next) {
    ...
    exp  = pkg_json["browserify"].exports,
    ...
    if (!req.query["__single"]){
      ...
      var is_allow = exp_scripts.concat(exp_styles).some(function(glob){
        glob = glob.replace(/^\.\//,"");
        return minimatch(path_relative,glob);
      });
      if (!is_allow) {
        readfile(filepath, res);
        return;
